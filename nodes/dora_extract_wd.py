"""
DoRA Extraction via SVD and Weight Decomposition.

Native implementation of Weight-Decomposed Low-Rank Adapter extraction
...
def extract_dora_from_files(
    ...
    print(f"[DoRA Extract] Preparing memory for {total_size_gb:.2f}GB operation...")
    ...
    try:
        ...
        def _process_layer(key):
            ...
            try:
                ...
                print(f"[DoRA Extract] Failed: {key}: {e}")
                del weight_diff
                return "skipped", None
            ...
            if mode_str == "full":
                ...
                lora_down, lora_up, rank_or_diff = result
                # rank is the size of lora_down's first dimension
                rank = lora_down.shape[0]
                layer_results[f"{lora_name}.lora_down.weight"] = lora_down.to(save_torch_dtype).cpu().contiguous()
                layer_results[f"{lora_name}.lora_up.weight"] = lora_up.to(save_torch_dtype).cpu().contiguous()
                layer_results[f"{lora_name}.alpha"] = torch.tensor(rank, dtype=save_torch_dtype)
...
class DoRAExtractFixed(io.ComfyNode):
    """Extract DoRA with fixed rank."""

    @classmethod
    def define_schema(cls):
        return io.Schema(
            node_id="DoRAExtractFixed",
            display_name="DoRA Extract (Fixed Rank)",
            category="ModelUtils/DoRA",
            description="Extract DoRA with specified fixed rank for each layer type.",
            inputs=[
                *_get_model_inputs(),
...
class DoRAExtractRatio(io.ComfyNode):
    """Extract DoRA with singular value ratio threshold."""

    @classmethod
    def define_schema(cls):
        return io.Schema(
            node_id="DoRAExtractRatio",
            display_name="DoRA Extract (Ratio)",
            category="ModelUtils/DoRA",
...
class DoRAExtractQuantile(io.ComfyNode):
    """Extract DoRA with cumulative singular value quantile."""

    @classmethod
    def define_schema(cls):
        return io.Schema(
            node_id="DoRAExtractQuantile",
            display_name="DoRA Extract (Quantile)",
            category="ModelUtils/DoRA",
...
class DoRAExtractKnee(io.ComfyNode):
    """Extract DoRA with automatic knee detection."""

    @classmethod
    def define_schema(cls):
        return io.Schema(
            node_id="DoRAExtractKnee",
            display_name="DoRA Extract (Knee Detection)",
            category="ModelUtils/DoRA",
...
class DoRAExtractFrobenius(io.ComfyNode):
    """Extract DoRA preserving target fraction of Frobenius norm."""

    @classmethod
    def define_schema(cls):
        return io.Schema(
            node_id="DoRAExtractFrobenius",
            display_name="DoRA Extract (Frobenius)",
            category="ModelUtils/DoRA",
