...
# =============================================================================
# Fast Resize via Base Model - Uses extraction code path for ~91x speedup
# =============================================================================

def resize_lora_via_base(
    lora_path: str,
    base_model_path: str,
    mode: str,
    linear_param: float,
    ...
    """
    Resize LoRA by merging to base model weights and re-extracting.

    This uses the fast extraction code path for ~91x speed improvement over
    the original merge-then-SVD approach.

    Args:
        lora_path: Path to input LoRA
        base_model_path: Path to base model the LoRA was trained on
        mode: "fixed", "ratio", "sv_fro", "sv_cumulative"
        linear_param: Rank for fixed mode, or mode parameter for dynamic
        conv_param: Same for conv layers
        device: Processing device
        save_dtype: Output dtype
        output_filename: Output filename (without extension)
        linear_max_rank: Max rank for linear layers (dynamic modes)
        conv_max_rank: Max rank for conv layers (dynamic modes)
        clamp_quantile: Quantile for weight clamping
        mismatch_mode: How to handle mismatches ("skip", "zeros", "error")
    ...
    # Prepare memory
    lora_size_gb = estimate_model_size(lora_path)
    base_size_gb = estimate_model_size(base_model_path)
    total_size_gb = lora_size_gb + base_size_gb

    if verbose:
        print(f"[LoRA Resize Fast] Preparing memory for {total_size_gb:.2f}GB operation...")
    prepare_for_large_operation(total_size_gb * 1.5, torch.device(device))

    # Open files with memory-efficient handlers
    lora_handler = MemoryEfficientSafeOpen(lora_path)
    base_handler = MemoryEfficientSafeOpen(base_model_path)

    try:
        lora_metadata = lora_handler.metadata().copy()
        lora_keys = lora_handler.keys()
        base_keys = set(base_handler.keys())

        # Detect LoRA format
        format_info = detect_lora_format(lora_keys)
        lora_pairs = extract_lora_pairs(lora_keys, format_info)
        network_dim, network_alpha = detect_lora_rank(lora_handler, lora_pairs)
        scale = network_alpha / network_dim if network_dim > 0 else 1.0

        if verbose:
            print(f"[LoRA Resize Fast] Format: {format_info['format']}, layers: {format_info['key_count']}")
            print(f"[LoRA Resize Fast] Original dim={network_dim}, alpha={network_alpha:.1f}")
            print(f"[LoRA Resize Fast] Mode: {mode}, linear_param={linear_param}, conv_param={conv_param}")

        # Compile skip patterns
        skip_patterns = _compile_patterns(skip_patterns_str)

        # Build mapping from base model keys to LoRA block names
        # LoRA keys are like: lora_unet_transformer_blocks_0_attn_to_q.lora_down.weight
        # Base keys are like: transformer_blocks.0.attn.to_q.weight
        down_suffix = format_info["down_suffix"]
        up_suffix = format_info["up_suffix"]
        alpha_suffix = format_info["alpha_suffix"]

        output_sd = {}
        stats = {"extracted": 0, "skipped": 0, "no_match": 0, "full": 0}

        # Common prefixes to strip from base model keys
        BASE_PREFIXES = ["model.diffusion_model.", "diffusion_model.", "transformer.", "model."]

        # Common prefixes to strip from LoRA block names (order matters - longer prefixes first)
        LORA_PREFIXES = [
        ...
        def extract_core_layer_base(key: str) -> str:
            """Extract core layer name from base model key (keeps dots)."""
        ...
        def extract_core_layer_lora(block_name: str) -> str:
            """Extract core layer name from LoRA block name and normalize to underscored format."""
            # Strip common prefixes
        ...
        for bk in base_handler.keys():
            if bk.endswith(".weight"):
                core = extract_core_layer_base(bk)
                # Convert dots to underscores for matching with LoRA format
                core_underscored = core.replace(".", "_")
                base_key_lookup[core_underscored] = bk

        if verbose:
            print(f"[LoRA Resize Fast] Base model has {len(base_key_lookup)} weight keys")
            # Debug: show first 3 samples from each side with their normalized forms
            sample_lora = list(lora_pairs.keys())[:3]
            sample_lora_normalized = [extract_core_layer_lora(b) for b in sample_lora]
            sample_base = list(base_key_lookup.items())[:3]
            print(f"[DEBUG] Sample LoRA blocks: {sample_lora}")
            print(f"[DEBUG] Sample LoRA normalized: {sample_lora_normalized}")
            print(f"[DEBUG] Sample base (underscored->original): {sample_base}")

        pbar = comfy.utils.ProgressBar(len(lora_pairs))

        with torch.no_grad():
            for block_name, block_keys in tqdm(lora_pairs.items(), desc="Resizing via extraction", unit="layers"):
                if "down" not in block_keys or "up" not in block_keys:
                    pbar.update(1)
                    continue

                # Check skip patterns on the LoRA block name
                if _matches_any_pattern(block_name, skip_patterns):
                    stats["skipped"] += 1
                    pbar.update(1)
                    continue

                # Extract core layer name from LoRA block (already underscored)
                core_lora = extract_core_layer_lora(block_name)

                # Look up the base model key
                if core_lora not in base_key_lookup:
                    if verbose and stats["no_match"] < 3:
                        print(f"[DEBUG] No match for LoRA core: {core_lora}")
                    stats["no_match"] += 1
                    pbar.update(1)
                    continue

                base_key = base_key_lookup[core_lora]

                # Load tensors with pinned memory for CUDA
                # Load base weight to RAM first
                cpu_base = base_handler.get_tensor(base_key)

                # Transfer to GPU if cuda selected
                if device == 'cuda':
                ...
                # Load LoRA weights
                cpu_down = lora_handler.get_tensor(block_keys["down"])
                cpu_up = lora_handler.get_tensor(block_keys["up"])
                if device == 'cuda':
                ...
                if "alpha" in block_keys:
                    alpha_tensor = lora_handler.get_tensor(block_keys["alpha"])
                    layer_alpha = float(alpha_tensor.item())
                else:
                    layer_alpha = float(network_dim)
                layer_scale = layer_alpha / network_dim if network_dim > 0 else 1.0

                # Compute LoRA delta: scale * (up @ down)
                is_conv = len(lora_down.shape) == 4

                if is_conv:
                    # Conv merge: scale * (up @ down reshaped)
                    in_rank, in_size, kernel_size, k_ = lora_down.shape
                    out_size, out_rank, _, _ = lora_up.shape
                    lora_delta = lora_up.reshape(out_size, -1) @ lora_down.reshape(in_rank, -1)
                    lora_delta = lora_delta.reshape(out_size, in_size, kernel_size, kernel_size)
                else:
                    # Linear merge: scale * (up @ down)
                    lora_delta = lora_up @ lora_down
                del lora_down, lora_up

                # Apply scale and merge into base to get finetuned weight
                finetuned_weight = base_weight + layer_scale * lora_delta
                del lora_delta

                # Subtract base to get the weight difference for extraction
                weight_diff = finetuned_weight - base_weight
                del base_weight, finetuned_weight

                # Extract new LoRA from the weight difference
                if is_conv:
                ...
                if mode_str == "full":
                    # Rank too high, store as full diff (rare)
                    stats["full"] += 1
                    pbar.update(1)
                    continue

                new_lora_down, new_lora_up, _ = result
                new_rank = new_lora_down.shape[0]
                new_alpha = float(new_rank)  # Standard: alpha = rank for 1.0 scale

                # Transfer extracted LoRA tensors back to RAM until saving
                output_sd[f"{block_name}{down_suffix}"] = new_lora_down.to(save_dtype).cpu().contiguous()
                output_sd[f"{block_name}{up_suffix}"] = new_lora_up.to(save_dtype).cpu().contiguous()
                output_sd[f"{block_name}{alpha_suffix}"] = torch.tensor([new_alpha]).to(save_dtype)

                # Clear remaining tensors from this layer
                del new_lora_down, new_lora_up, result

                stats["extracted"] += 1
                pbar.update(1)

        if verbose:
            print(f"[LoRA Resize Fast] Done: {stats['extracted']} extracted, {stats['skipped']} skipped, {stats['full']} full")

        if not output_sd:
            raise ValueError("No LoRA weights extracted. Check that the base model matches the LoRA.")

        # Update metadata
        lora_metadata["ss_training_comment"] = f"Resized via base model extraction, mode={mode}"
        if mode == "fixed":
            lora_metadata["ss_network_dim"] = str(int(linear_param))
            lora_metadata["ss_network_alpha"] = str(int(linear_param))
        else:
            lora_metadata["ss_network_dim"] = "Dynamic"
            lora_metadata["ss_network_alpha"] = "Dynamic"
        ...
        save_file(output_sd, output_path, lora_metadata)
        print(f"[LoRA Resize Fast] Saved to {output_path}")

        return output_path

    finally:
        lora_handler.__exit__(None, None, None)
        base_handler.__exit__(None, None, None)
        cleanup_after_operation()


# =============================================================================
# Fast Resize Node Definitions
# =============================================================================

class LoRAResizeViaBaseFixed(io.ComfyNode):
    """Fast LoRA resize using base model + extraction path."""

    @classmethod
    def define_schema(cls):
        return io.Schema(
            node_id="LoRAResizeViaBaseFixed",
            display_name="LoRA Resize Fast (Fixed Rank)",
            category="ModelUtils/LoRA/Resize",
            description="Fast LoRA resize by merging to base model and re-extracting. ~91x faster than standard resize.",
            inputs=[
                io.Combo.Input("lora_name", options=folder_paths.get_filename_list("loras"),
                              tooltip="LoRA to resize"),
                io.Combo.Input("base_model", options=folder_paths.get_filename_list("diffusion_models"),
                              tooltip="Base model the LoRA was trained on"),
                io.Int.Input("linear_dim", default=64, min=1, max=4096,
                            tooltip="Target rank for linear/attention layers"),
                io.Int.Input("conv_dim", default=32, min=1, max=4096,
                            tooltip="Target rank for conv layers"),
                io.Int.Input("svd_niter", default=2, min=0, max=10,
                            tooltip="SVD power iterations (higher = more accurate)"),
                ...
                io.Combo.Input("mismatch_mode", options=["skip", "zeros", "error"], default="skip",
                              tooltip="How to handle key mismatches"),
                io.String.Input("skip_patterns", default="", multiline=True,
                               tooltip="Regex patterns for layers to skip"),
                io.String.Input("output_filename", default="resized_lora_fast"),
                io.Combo.Input("save_dtype", options=["fp16", "bf16", "fp32"], default="fp16"),
                io.Combo.Input("device", options=["cuda", "cpu"], default="cuda"),
            ],
            outputs=[io.String.Output(display_name="output_path")],
    ...
    @classmethod
    def execute(cls, lora_name, base_model, linear_dim, conv_dim, svd_niter,
                clamp_quantile, mismatch_mode, skip_patterns,
                output_filename, save_dtype, device) -> io.NodeOutput:
        lora_path = folder_paths.get_full_path_or_raise("loras", lora_name)
        base_path = folder_paths.get_full_path_or_raise("diffusion_models", base_model)
        dtype = {"fp16": torch.float16, "bf16": torch.bfloat16, "fp32": torch.float32}[save_dtype]

        path = resize_lora_via_base(
            lora_path=lora_path,
            ...
            linear_max_rank=linear_dim,
            conv_max_rank=conv_dim,
...
class LoRAResizeViaBaseRatio(io.ComfyNode):
    """Fast LoRA resize keeping SVs above ratio threshold."""

    @classmethod
    def define_schema(cls):
        return io.Schema(
            node_id="LoRAResizeViaBaseRatio",
            display_name="LoRA Resize Fast (SV Ratio)",
            category="ModelUtils/LoRA/Resize",
            description="Fast dynamic resize keeping singular values where S[i] > S[0]/ratio.",
            inputs=[
                io.Combo.Input("lora_name", options=folder_paths.get_filename_list("loras"),
                              tooltip="LoRA to resize"),
                io.Combo.Input("base_model", options=folder_paths.get_filename_list("diffusion_models"),
                ...
                io.String.Input("skip_patterns", default="", multiline=True,
                               tooltip="Regex patterns for layers to skip"),
                io.String.Input("output_filename", default="resized_lora_ratio_fast"),
                io.Combo.Input("save_dtype", options=["fp16", "bf16", "fp32"], default="fp16"),
                io.Combo.Input("device", options=["cuda", "cpu"], default="cuda"),
            ],
            outputs=[io.String.Output(display_name="output_path")],
    ...
    @classmethod
    def execute(cls, lora_name, base_model, linear_max_rank, conv_max_rank, ratio,
                clamp_quantile, mismatch_mode, skip_patterns,
                output_filename, save_dtype, device) -> io.NodeOutput:
        lora_path = folder_paths.get_full_path_or_raise("loras", lora_name)
        base_path = folder_paths.get_full_path_or_raise("diffusion_models", base_model)
...
class LoRAResizeViaBaseFrobenius(io.ComfyNode):
    """Fast LoRA resize preserving Frobenius norm target."""

    @classmethod
    def define_schema(cls):
        return io.Schema(
            node_id="LoRAResizeViaBaseFrobenius",
            display_name="LoRA Resize Fast (Frobenius)",
            category="ModelUtils/LoRA/Resize",
            description="Fast dynamic resize preserving target fraction of Frobenius norm.",
            inputs=[
                io.Combo.Input("lora_name", options=folder_paths.get_filename_list("loras"),
                              tooltip="LoRA to resize"),
                io.Combo.Input("base_model", options=folder_paths.get_filename_list("diffusion_models"),
                ...
                io.String.Input("skip_patterns", default="", multiline=True,
                               tooltip="Regex patterns for layers to skip"),
                io.String.Input("output_filename", default="resized_lora_fro_fast"),
                io.Combo.Input("save_dtype", options=["fp16", "bf16", "fp32"], default="fp16"),
                io.Combo.Input("device", options=["cuda", "cpu"], default="cuda"),
            ],
            outputs=[io.String.Output(display_name="output_path")],
    ...
    @classmethod
    def execute(cls, lora_name, base_model, linear_max_rank, conv_max_rank, target,
                clamp_quantile, mismatch_mode, skip_patterns,
                output_filename, save_dtype, device) -> io.NodeOutput:
        lora_path = folder_paths.get_full_path_or_raise("loras", lora_name)
        base_path = folder_paths.get_full_path_or_raise("diffusion_models", base_model)
...
class LoRAResizeViaBaseCumulative(io.ComfyNode):
    """Fast LoRA resize preserving cumulative SV target."""

    @classmethod
    def define_schema(cls):
        return io.Schema(
            node_id="LoRAResizeViaBaseCumulative",
            display_name="LoRA Resize Fast (Cumulative)",
            category="ModelUtils/LoRA/Resize",
            description="Fast dynamic resize preserving target fraction of cumulative singular values.",
            inputs=[
                io.Combo.Input("lora_name", options=folder_paths.get_filename_list("loras"),
                              tooltip="LoRA to resize"),
                io.Combo.Input("base_model", options=folder_paths.get_filename_list("diffusion_models"),
                ...
                io.String.Input("skip_patterns", default="", multiline=True,
                               tooltip="Regex patterns for layers to skip"),
                io.String.Input("output_filename", default="resized_lora_cumulative_fast"),
                io.Combo.Input("save_dtype", options=["fp16", "bf16", "fp32"], default="fp16"),
                io.Combo.Input("device", options=["cuda", "cpu"], default="cuda"),
            ],
            outputs=[io.String.Output(display_name="output_path")],
...
# =============================================================================
# Multi-LoRA Merge to Single LoRA
# =============================================================================

def merge_multi_loras_via_base(
    lora_paths: List[str],
    ...
    """
    Merge multiple LoRAs to base model and re-extract as a single LoRA.

    Args:
        lora_paths: List of paths to LoRA files
        lora_weights: List of weight strengths (0.0-1.0) for each LoRA
        ...
        linear_max_rank: Max rank for linear layers (dynamic modes)
        conv_max_rank: Max rank for conv layers (dynamic modes)
        clamp_quantile: Clamp outlier singular values
        skip_patterns_str: Regex patterns for layers to skip
        svd_niter: Power iterations for SVD accuracy
        verbose: Print progress info

    Returns:
        Path to saved merged LoRA
    """
    from .lora_extract_svd import _svd_extract_linear, _svd_extract_conv
    ...
    if verbose:
        print(f"[Multi-LoRA Merge] Preparing memory for {total_size_gb:.2f}GB operation...")
        print(f"[Multi-LoRA Merge] Merging {len(lora_paths)} LoRAs with weights: {lora_weights}")
    prepare_for_large_operation(total_size_gb * 1.5, torch.device(device))
    ...
    try:
        ...
        for i, handler in enumerate(lora_handlers):
            ...
            all_block_names.update(pairs.keys())
            if verbose:
                print(f"[Multi-LoRA Merge] LoRA {i+1}: {format_info['format']}, {len(pairs)} layers, dim={network_dim}")

        # Use first LoRA's format for output
        output_format = lora_infos[0]["format_info"]
        down_suffix = output_format["down_suffix"]
        up_suffix = output_format["up_suffix"]
        alpha_suffix = output_format["alpha_suffix"]
        ...
        if verbose:
            print(f"[Multi-LoRA Merge] Base model has {len(base_key_lookup)} weight keys")
        ...
        output_sd = {}
        stats = {"extracted": 0, "skipped": 0, "no_match": 0, "full": 0}
        pbar = comfy.utils.ProgressBar(len(all_block_names))

        with torch.no_grad():
            for block_name in tqdm(sorted(all_block_names), desc="Merging LoRAs", unit="layers"):
                # Check skip patterns
                if _matches_any_pattern(block_name, skip_patterns):
                    stats["skipped"] += 1
                    pbar.update(1)
                ...
                # Extract core layer name
                core_lora = extract_core_layer_lora(block_name)

                # Find base model key
                if core_lora not in base_key_lookup:
                    if verbose and stats["no_match"] < 3:
                        print(f"[DEBUG] No match for: {core_lora}")
                    stats["no_match"] += 1
                    pbar.update(1)
                    continue
                ...
                # Accumulate deltas from all LoRAs that have this block
                combined_delta = None
                is_conv = None

                for info in lora_infos:
                    if block_name not in info["pairs"]:
                        continue
                    ...
                    if combined_delta is None:
                        combined_delta = effective_scale * delta
                    else:
                        combined_delta = combined_delta + effective_scale * delta
                    del delta

                if combined_delta is None:
                    # No LoRA had this block
                    del base_weight
                    pbar.update(1)
                    continue

                # Compute finetuned weight
                finetuned_weight = base_weight + combined_delta

                # Compute weight difference for extraction
                weight_diff = finetuned_weight - base_weight
                del base_weight, finetuned_weight, combined_delta
        ...
        if verbose:
            print(f"[Multi-LoRA Merge] Done: {stats['extracted']} extracted, {stats['skipped']} skipped, {stats['no_match']} no_match")

        if not output_sd:
            raise ValueError("No LoRA weights extracted. Check that base model matches the LoRAs.")

        # Build metadata
        metadata = {
            "ss_training_comment": f"Merged {len(lora_paths)} LoRAs via base model, mode={mode}",
            "ss_network_dim": str(int(linear_param)) if mode == "fixed" else "Dynamic",
            "ss_network_alpha": str(int(linear_param)) if mode == "fixed" else "Dynamic",
        }
        ...
        save_file(output_sd, output_path, metadata)
        print(f"[Multi-LoRA Merge] Saved to {output_path}")
...
class LoRAMultiMerge(io.ComfyNode):
    """Merge multiple LoRAs to base model and extract as single LoRA."""
    ...
    def define_schema(cls):
        return io.Schema(
            ...
            description="Merge 1-4 LoRAs to base model and re-extract as a single LoRA.",
            inputs=[
                io.Combo.Input("base_model", options=folder_paths.get_filename_list("diffusion_models"),
                ...
                # Extraction settings
                io.Combo.Input("mode", options=["fixed", "sv_ratio", "sv_fro", "sv_cumulative"], default="fixed",
                              tooltip="Rank selection mode"),
                io.Int.Input("linear_rank", default=64, min=1, max=1024,
                            tooltip="Target rank for linear layers (fixed) or max rank (dynamic)"),
                io.Int.Input("conv_rank", default=32, min=1, max=512,
                            tooltip="Target rank for conv layers (fixed) or max rank (dynamic)"),
                io.Float.Input("dynamic_param", default=0.9, min=0.1, max=1.0, step=0.01,
                              tooltip="Parameter for dynamic modes (ratio/target)"),
                io.Float.Input("clamp_quantile", default=0.99, min=0.5, max=1.0, step=0.01,
                              tooltip="Clamp outlier singular values"),
                io.String.Input("skip_patterns", default="", multiline=True,
                               tooltip="Regex patterns for layers to skip"),
    ...
    def execute(cls, base_model, lora_count,
                ...
                mode, linear_rank, conv_rank, dynamic_param,
                clamp_quantile, skip_patterns, output_filename, save_dtype, device) -> io.NodeOutput:
        ...
        if mode == "fixed":
            linear_param = float(linear_rank)
            conv_param = float(conv_rank)
            linear_max_rank = None
            conv_max_rank = None
        else:
            linear_param = dynamic_param
            conv_param = dynamic_param
            linear_max_rank = linear_rank
            conv_max_rank = conv_rank

        path = merge_multi_loras_via_base(
